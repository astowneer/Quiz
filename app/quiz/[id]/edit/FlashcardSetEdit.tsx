...
export const FlashcardSetEdit = ({ categoryId, categoryName, editedFlashcards }: FlashcardSetEditProps) => {
  ...
  const [state, formAction, isPending] = useActionState(editFlashcardSet, initialState);
  ...
  function updateFlashcard(index: number, event: React.ChangeEvent<HTMLTextAreaElement>) {
    ...
    setFlashcards((prev) =>
      prev.map((flashcard, i) =>
        i === index ? { ...flashcard, [name]: value, id: flashcard.id } : flashcard
      )
    );
  }  
  ...
  function insertFlashcard(index: number) {
    setFlashcards((prevList) => {
      ...
      updatedList.splice(index + 1, 0, newFlashcard);
      return updatedList;
    });
  }
  ...
  return (
    <article className="w-[800px] m-auto space-y-5 py-10">
      <h2 className="text-2xl font-bold mb-5">Edit Flashcards</h2>
          ...
          <Fragment key={index} >
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-10 px-10 py-5">
                ...
                <input type="hidden" name={`flashcards[${index}][term]`} value={flashcard.title} />
                ...
                <textarea
                  ...
                  onChange={(event) => updateFlashcard(index, event)}
                />
                <textarea
                  className="h-8 bg-transparent resize-none placeholder:text-white/70 overflow-hidden border-b-4 outline-none border-red-900 focus:border-white" 
                  name="definition"
